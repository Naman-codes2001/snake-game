...
class SnakeGame extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      blockColor: this.props.blockColor || this.getRandomColor(),
      score: 0,
  ...
  initGame() {
    ...
    // initialization position of block
    let blockXpos =
    ...
    this.setState({
      ...
      startSnakeSize,
      snake,
      block: { Xpos: blockXpos, Ypos: blockYpos },
    });
  }
  ...
  resetGame() {
    ...
    while (this.isBlockOnSnake(block.Xpos, block.Ypos)) {
      block.Xpos =
        Math.floor(Math.random() * ((width - blockWidth) / blockWidth + 1)) *
        blockWidth;
      block.Ypos =
  ...
  tryToEatBlock() {
    ...
    // if snake's head is on block
    if (snake[0].Xpos === block.Xpos && snake[0].Ypos === block.Ypos) {
      let width = this.state.width;
      let height = this.state.height;
      let blockWidth = this.state.blockWidth;
      let blockHeight = this.state.blockHeight;
      let newTail = { Xpos: block.Xpos, Ypos: block.Ypos };
  ...
  isBlockOnSnake(blockXpos, blockYpos) {
    let snake = this.state.snake;
    for (let i = 0; i < snake.length; i++) {
      if (blockXpos === snake[i].Xpos && blockYpos === snake[i].Ypos)
        return true;
    }
    return false;
  }
      ...
      >
        ...
        <div
          ...
          style={{
            width: this.state.blockWidth,
            height: this.state.blockHeight,
            left: this.state.block.Xpos,
            top: this.state.block.Ypos,
            background: this.state.blockColor,
          }}
        />
        <div id="Score" style={{ fontSize: this.state.width / 20 }}>
          SCORE: {this.state.score}&ensp;&ensp;&ensp;&ensp;&ensp;&ensp; HIGH
